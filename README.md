...
- 新增: 编辑模式新增Ctrl+H查找替换，新增配对标记补全（自动/环绕/跳过/成对删除）
  - 自动补全：在编辑模式输入 `(`、`[`、`{`、`"`、`'`、`` ` ``、`*`、`_` 等左侧标记时，自动插入右侧标记并将光标置于两者之间。*个别中文输入法中可能有兼容问题，我无法一一测试*
  - 环绕补全：选中文本后输入上述任一左侧标记，自动在选中文本前后添加配对标记，仅调整选区以便继续输入。*个别中文输入法中可能有兼容问题，我无法一一测试*
  - 跳过右侧：输入右侧标记时，若光标处已存在相同标记，直接右移光标避免重复。*个别中文输入法中可能有兼容问题，我无法一一测试*
  - 成对删除：Backspace 位于一对配对标记之间时，一次性删除成对标记。*个别中文输入法中可能有兼容问题，我无法一一测试*
